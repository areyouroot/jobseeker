...
class ConfigLoader:
    """Reads and validates config.ini settings for LinkedIn Auto-Applier."""
    ...
    def load(cls) -> dict:
        ...
        Returns a dictionary with keys:
            - email, password, resume_path, slow_mo, timeout, apply_delay,
              max_applications, schedule_times
            - telegram_bot_token, telegram_chat_id (optional, can be empty)
            - llm_url (optional)
            - filters (dict with optional location, experience_level, date_posted, remote)
        """
        ...
        try:
            ...
            max_applications = config.getint("settings", "max_applications", fallback=25)
        ...
        # Filters config (optional)
        filters = {}
        if config.has_section("filters"):
            for key in ["location", "experience_level", "date_posted", "remote"]:
                val = config.get("filters", key, fallback="").strip()
                if val:
                    filters[key] = val
