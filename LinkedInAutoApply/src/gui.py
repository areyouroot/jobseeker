...
from src.scheduler import ApplyScheduler
from src.orchestrator import ApplyOrchestrator
from src.config import CONFIG_FILE

class LinkedInApplierApp:
    """
    Desktop GUI with interactive buttons, live log, status dashboard,
    and hover/active visual feedback for LinkedIn Easy Apply automation.
    """

    def __init__(self, cfg):
        self.cfg = cfg
        self.scheduler = ApplyScheduler(cfg, log_fn=self.log, status_fn=self._set_activity)
        self._current_activity = "idle"  # idle | running | applying

        # Main window
        self.root = tk.Tk()
        self.root.title("LinkedIn Easy Apply Auto-Applier")
    ...
    def _build_ui(self):
        ...
        tk.Label(
            title_bar, text="⚡ LinkedIn Easy Apply Auto-Applier",
            font=("Segoe UI", 20, "bold"), fg=Colors.ACCENT_BLUE, bg=Colors.BG_CARD,
        ...
        # Card 2: Next run
        self.next_card = self._make_card(cards_frame, "NEXT RUN", "--:--", Colors.ACCENT_BLUE)
        ...
        # Card 4: Applied today
        self.count_card = self._make_card(cards_frame, "RUNS TODAY", "0", Colors.ACCENT_GREEN)
        self.count_card["frame"].grid(row=0, column=3, padx=6, sticky="nsew")

        # Card 5: Last run
        self.last_card = self._make_card(cards_frame, "LAST RUN", "Never", Colors.ACCENT_ORANGE)
        ...
        if self.cfg.get("telegram_bot_token") and self.cfg.get("telegram_chat_id"):
            self.log("[INFO] 📱 Telegram bot configured — questions will be relayed.")
        else:
            self.log("[WARNING] Telegram not configured — screening questions will be skipped.")
        self.log("[INFO] Click ▶ Start to begin scheduled runs, or ⚡ Run Now for an immediate cycle.")
    ...
    def _update_dashboard(self, now):
        ...
        # Next run time
        next_t = self._get_next_run(now)
        self.next_card["value"].configure(text=next_t if next_t else "Done for today")

        # Countdown timer — live remaining time until next run
        countdown_str = self._get_countdown(now)
        self.countdown_card["value"].configure(text=countdown_str)

        # Run count
        self.count_card["value"].configure(text=str(self.scheduler.run_count))

        # Last run
        if self.scheduler.last_run_time:
            self.last_card["value"].configure(
                text=self.scheduler.last_run_time.strftime("%H:%M:%S"))
        else:
            self.last_card["value"].configure(text="Never")

    def _get_next_run(self, now):
    ...
    def _get_countdown(self, now):
        """Calculate remaining time until the next scheduled run."""
    ...
    def _update_status_indicator(self):
        ...
        if self._current_activity == "applying":
            self.status_card["value"].configure(text="● Applying...", fg=Colors.ACCENT_BLUE)
    ...
    def _on_start(self):
        """Start the scheduler — run one cycle immediately AND start the scheduler."""
        ...
        # Also do an immediate run in a background thread
        self.log("[INFO] ▶ Starting — running initial Easy Apply cycle now...")
        orchestrator = ApplyOrchestrator(self.cfg, self.log, self._set_activity)
    ...
    def _on_run_now(self):
        """Trigger an immediate apply cycle in a background thread."""
        self.log("[INFO] ⚡ Manual apply cycle triggered!")
        orchestrator = ApplyOrchestrator(self.cfg, self.log, self._set_activity)
        thread = threading.Thread(
            target=orchestrator.run_once,
            daemon=True,
