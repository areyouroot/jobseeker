...
class ApplyScheduler:
    """
    Background scheduler — checks time every 10 seconds, fires apply cycles
        ...
        while not self._stop_event.is_set():
            ...
            for t in self.cfg["schedule_times"]:
                ...
                if 0 <= diff < 120:
                    ...
                    self.log_fn(f"\n[TRIGGER] ⏰ Scheduled run at {t}!")
                    try:
                        orchestrator = ApplyOrchestrator(self.cfg, self.log_fn, self.status_fn)
                        orchestrator.run_once()
                        self.run_count += 1
                        self.last_run_time = datetime.now()
                    except Exception as e:
                        self.log_fn(f"[ERROR] Apply cycle failed: {e}")
